...
## 📅 บันทึกวันที่ 22 มิถุนายน 2569 — เฟส 2 / D2 (ล่าสุด)
...
### ⚠️ ขั้นที่ผู้ใช้ต้องทำเอง (โค้ดพร้อมแล้ว — ยังไม่ได้ลงจริงใน Supabase)
เปิด Supabase project `pd-monitor` → **SQL Editor** → paste รัน **ตามลำดับ**:
`0001_schema.sql` → `0002_audit_log.sql` → `0003_rls.sql` → `seed.sql`
(รายละเอียดเต็มใน `web/supabase/README.md`) → เช็ก Table Editor เห็น 8 ตาราง + audit_log มี log seed
...
**เฟส 2 / D2 = เขียน schema เสร็จแล้ว ✅ (22 มิ.ย. 69)** — ไฟล์ SQL migration + seed + คู่มือ อยู่ใน `web/supabase/`
  ⏳ **รอผู้ใช้ paste ลง Supabase SQL Editor** (0001→0002→0003→seed) แล้วเช็ก Table Editor ว่าครบ 8 ตาราง
**ถัดไป:** หลัง paste สำเร็จ → เริ่ม **D3** — auth + role + `has_role` helper · PWA manifest · responsive · RLS policy แยก role
  (อ่าน `docs/recommendations.md` B/C + fetch Notion เช็ก requirement ใหม่ก่อน)
