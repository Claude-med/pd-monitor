...
## 📅 บันทึกวันที่ 23 มิถุนายน 2569 — เฟส 8 / D8 ส่วน 2 (ก้อน 1/4): เทส RLS (B4) (ล่าสุด)
...
- **paste `tests/rls_impersonation_test.sql`** ลง Supabase SQL Editor (รันหลัง 0001–0009 + seed + ผูก Auth ครบ)
  → ดูตารางผลลัพธ์ ทุกแถวควร ✅ · ถ้าเจอ ❌ ส่งภาพ/ข้อความกลับมา จะได้ไล่ดู policy ที่หลุด
- หมายเหตุ: ถ้า SQL Editor บ่นเรื่อง `begin`/`rollback` (transaction mode) ให้บอก จะปรับให้
...
**เฟส 8 / D8 = กำลังทำ (แบ่งก้อน) — ส่วน 1 Realtime เสร็จ ✅ (22 มิ.ย. 69)**
  ...
  · **ส่วน 2 = กำลังทำ (แบ่ง 4 ก้อน):**
    - **ก้อน 1 เทส RLS (B4) เสร็จ ✅ (23 มิ.ย. 69)** — `web/supabase/tests/rls_impersonation_test.sql`
      (impersonation + begin/rollback ไม่แตะข้อมูล · เทส read/write/audit/anon ครบ · แยก RLS vs grant) · ⏳ รอ paste + รอผล
    - ก้อนถัดไป: groundwork offline (C1) · ขัดเกลามือถือ · perf (index/query)
