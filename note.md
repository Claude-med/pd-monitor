...
## 📅 บันทึกวันที่ 23 มิถุนายน 2569 — เฟส 8 / D8 ส่วน 2 (ก้อน 2/4): Offline-resilient save (C1) (ล่าสุด)
...
- **paste `0010_record_idempotency.sql`** ลง Supabase (ต่อจาก 0001–0009) — **สำคัญ:** ถ้ายังไม่ paste
  การบันทึกผลผลิตจะ error (rpc ใหม่มี `p_client_id` ที่ฟังก์ชันเดิมไม่มี) → ต้อง paste ก่อนถึงจะบันทึกได้
- ทดสอบ: login production เปิดงาน in_production → เปิด DevTools (มือถือ: ปิด wifi) ตั้ง Network=Offline →
  กดบันทึก → เห็น "เน็ตมีปัญหา กำลังลองใหม่" → เปิดเน็ตกลับ → ขึ้นในตาราง (ไม่ซ้ำ) · ลองกดบันทึกรัวๆ ดูว่าไม่เกิดแถวซ้ำ
...
**เฟส 8 / D8 = กำลังทำ (แบ่งก้อน) — ส่วน 1 Realtime เสร็จ ✅ (22 มิ.ย. 69)**
  ...
  · **ส่วน 2 = กำลังทำ (แบ่ง 4 ก้อน):**
    ...
    - **ก้อน 2 Offline-resilient save (C1) เสร็จ ✅ (23 มิ.ย. 69)** — `0010_record_idempotency.sql` (client_id idempotency)
      + `lib/offline-queue.ts` + ยกเครื่อง `record-form.tsx` (auto-retry/backoff · คิว localStorage · banner กู้ค้าง · online retry)
      · build ผ่าน · ⏳ **รอ paste 0010** + รอทดสอบ UI (offline mode)
    - ก้อนถัดไป: ขัดเกลามือถือ (touch target) · perf (index/query)
