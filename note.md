...
## 📅 บันทึกวันที่ 23 มิถุนายน 2569 — เฟส 9 / D9 (ก้อน 1): หน้าสร้างงานผลิตใหม่ (ล่าสุด)
...
- **paste `0011_create_job.sql`** ลง Supabase (ต่อจาก 0001–0010) — ถ้ายังไม่ paste ปุ่มสร้างงานจะ error
- ทดสอบ: login **manager** → บอร์ดงาน → "＋ สร้างงานใหม่" → กรอก (ลองเพิ่มยาใหม่ด้วย) → สร้าง → เด้งเข้าหน้างานใหม่
  · login role อื่น (production) → ไม่เห็นปุ่ม + เข้า `/board/new` ตรงๆ ก็ขึ้น "ไม่มีสิทธิ์"
...
**เฟส 9 / D9 = กำลังทำ (23 มิ.ย. 69):**
  · **ก้อน 1 หน้าสร้างงานผลิตใหม่ เสร็จ ✅** — `0011_create_job.sql` (`create_product`+`create_job_with_order`, manager เท่านั้น)
    + `/board/new` (ฟอร์ม + เพิ่มยา inline) + ปุ่มบนบอร์ด · build ผ่าน · ⏳ **รอ paste 0011** + รอทดสอบ UI
    (ปิด gap requirement ข้อ 1 ก่อน UAT) · ถัดไป: checklist UAT + ตรวจ go-live
